...
export default function EditRestaurantPage() {
  ...
  async function loadExperience() {
    const { data } = await supabase
      .from('experiences')
      .select(`*, restaurant_details(*), photos(*)`)
      .eq('id', params.id)
      .single()
    ...
    if (data) {
      ...
      if (data.restaurant_details) {
        setCuisine(data.restaurant_details.cuisine || '')
        setPriceRange(data.restaurant_details.price_range || '')
        setAddress(data.restaurant_details.address || '')
        setWebsite(data.restaurant_details.website || '')
        setBrettRating(data.restaurant_details.brett_rating)
        setJeanRating(data.restaurant_details.jean_rating)
        setDishesOrdered(data.restaurant_details.dishes_ordered || '')
        setCost(data.restaurant_details.cost?.toString() || '')
      }
  ...
  async function handleSave() {
    ...
    try {
      ...
      await supabase.from('restaurant_details').update({
      ...
      <main className="max-w-2xl mx-auto p-8">
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Dishes Ordered</label>
